...
import { Activity, Radio, BarChart3, Clock, AlertTriangle } from 'lucide-react';
    ...
    <div className="py-8 px-4 sm:px-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-[#06101F] border border-[#1E3C5C] space-y-1">
          <span className="font-mono text-[11px] text-[#8D9AAA]">SCANS RECORDED</span>
        ...
        <div className="p-4 bg-[#06101F] border border-[#1E3C5C] space-y-1">
          <span className="font-mono text-[11px] text-[#8D9AAA]">MEDIAN SCAN DURATION</span>
          <div className="font-headline font-bold text-2xl text-[#00CFFF]">{loading ? '—' : `${metrics.medianMs} ms`}</div>
          <span className="font-mono text-[10px] text-[#65E7FF]">Across all stored scans</span>
        </div>

        <div className="p-4 bg-[#06101F] border border-[#1E3C5C] space-y-1">
          <span className="font-mono text-[11px] text-[#8D9AAA]">HIGH-RISK SCANS (≥ 7.0)</span>
          ...
          <table className="w-full text-left font-mono text-xs">
            ...
            <tbody className="divide-y divide-[#1E3C5C]/50 text-[#D7DEE7]">
              {telemetryPoints.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-6 px-3 text-center text-[#8D9AAA]">
              ...
              {telemetryPoints.map((pt, i) => (
                <tr key={i} className="hover:bg-[#06101F] transition-colors">
                  <td className="py-2.5 px-3 text-[#8D9AAA]">{pt.time}</td>
                  <td className="py-2.5 px-3 font-semibold text-white">{pt.event}</td>
                  <td className="py-2.5 px-3 text-[#00CFFF]">{pt.repo}</td>
                  <td className="py-2.5 px-3">
                    <span className="px-1.5 py-0.5 text-[9px] font-bold rounded bg-[#00E699]/20 text-[#00E699]">
                      {pt.status}
                    </span>
                  </td>
                  <td className="py-2.5 px-3 text-right text-[#65E7FF]">{pt.latency}</td>
                </tr>
              ))}
            </tbody>
          </table>
