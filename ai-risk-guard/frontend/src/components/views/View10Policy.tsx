...
import { ShieldCheck, Lock, Sliders, CheckCircle2, AlertOctagon, Save } from 'lucide-react';
    ...
    <div className="py-8 px-4 sm:px-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#1E3C5C]">
        ...
        <div className="flex items-center space-x-3">
          <div className="px-3 py-1.5 bg-[#030C22] border border-[#184384] font-mono text-[10px] text-[#8D9AAA]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <GlassPanel
          ...
          accentColor={blockedModules.length ? '#FF304F' : '#00E699'}
        >
          <div className="space-y-3 font-mono text-xs">
            ...
            <div className="space-y-2">
              {blockedModules.length === 0 && (
                <div className="p-2.5 bg-[#02050B] border border-[#1E3C5C] text-[#8D9AAA] text-[10px]">
                  No banned modules configured.
                </div>
              )}
              {blockedModules.map((m) => (
                <div key={m.name} className="p-2.5 bg-[#02050B] border border-[#1E3C5C] flex items-center justify-between">
                  <div>
                    <span className="text-white font-bold">{m.name}</span>
                    <span className="text-[10px] text-[#8D9AAA] block">{m.reason}</span>
                  </div>
                  <span className="px-2 py-0.5 bg-[#8F1424]/40 text-[#FF304F] border border-[#FF304F] text-[10px] font-bold">
                    BLOCKED
                  </span>
                </div>
              ))}
        ...
        <GlassPanel
          ...
          accentColor={forbiddenFuncs.length ? '#FF304F' : '#00E699'}
        >
          <div className="space-y-3 font-mono text-xs">
            <p className="text-[#8D9AAA] text-[11px]">
            ...
            <div className="space-y-2">
              ...
              {forbiddenFuncs.map((f) => (
                <div key={f.name} className="p-2.5 bg-[#02050B] border border-[#1E3C5C] flex items-center justify-between">
                  <div>
                    <span className="text-white font-bold">{f.name}</span>
                    <span className="text-[10px] text-[#65E7FF] block">{f.replacement}</span>
                  </div>
                  <span className="px-2 py-0.5 bg-[#8F1424]/40 text-[#FF304F] border border-[#FF304F] text-[10px] font-bold">
                    BANNED
                  </span>
        ...
        >
          <div className="space-y-3 font-mono text-xs">
            ...
            <div className="space-y-2">
              ...
              {sanitizers.map((s) => (
                <div key={s.type} className="p-2.5 bg-[#02050B] border border-[#1E3C5C] flex items-center justify-between">
                  <div>
        ...
        >
          <div className="space-y-4 font-mono text-xs">
            ...
            <div className="p-3 bg-[#02050B] border border-[#1E3C5C] space-y-2">
              {sensitivePaths.length === 0 && (
                <div className="text-[#8D9AAA] text-[10px]">No sensitive paths configured.</div>
              )}
              {sensitivePaths.map((p) => (
                <div key={p} className="flex items-center justify-between">
                  <span className="text-[#00CFFF] font-mono">{p}/</span>
                  <span className="px-2 py-0.5 bg-[#087BFF]/20 text-[#00CFFF] border border-[#00CFFF] text-[10px] font-bold">
                    ELEVATED
                  </span>
                </div>
              ))}
