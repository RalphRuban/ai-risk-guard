...
import { Terminal, Shield, Play, AlertTriangle, CheckCircle, Code, Cpu } from 'lucide-react';
...
export const View06Scanner: React.FC<View06ScannerProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="py-8 px-4 sm:px-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-2 overflow-x-auto pb-2 border-b border-[#1E3C5C]/50 font-mono text-xs">
        <span className="text-[#8D9AAA] text-[11px] shrink-0">BENCHMARK PRESETS:</span>
          ...
          <button
            ...
            className={`px-3 py-1.5 shrink-0 border transition-all text-[11px] ${
              selectedBenchmark === key
                ? 'bg-[#087BFF]/30 border-[#00CFFF] text-[#65E7FF] font-semibold'
                : 'bg-[#06101F] border-[#1E3C5C] text-[#8D9AAA] hover:text-white hover:border-[#00CFFF]/40'
            }`}
          >
            {key}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7">
          ...
          >
            <div className="relative font-mono text-xs bg-[#02050B] border border-[#1E3C5C] p-4 min-h-[340px] flex">
              {/* Line Numbers */}
              <div className="select-none text-[#8D9AAA]/40 pr-4 text-right space-y-1.5 border-r border-[#1E3C5C]/50">
                {code.split('\n').map((_, idx) => (
              ...
              <textarea
                ...
                className="w-full pl-4 bg-transparent border-none outline-none text-[#F1F5F9] resize-none font-mono leading-normal"
                rows={12}
                spellCheck={false}
              />
            </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <GlassPanel
            ...
            accentColor={scanResult.hasVuln ? '#FF304F' : '#00E699'}
          >
            {scanResult.hasVuln ? (
              <div className="space-y-4 font-mono text-xs">
                {/* Finding Summary Card */}
                <div className="p-3 bg-[#8F1424]/30 border border-[#FF304F] space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="px-2 py-0.5 bg-[#FF304F] text-white font-bold text-[10px]">
                      CRITICAL FINDING
                    </span>
                    <span className="text-[#FF304F] font-bold">{scanResult.cwe}</span>
                  </div>
                  <h4 className="font-headline font-bold text-sm text-white">{scanResult.name}</h4>
                  <p className="text-[11px] text-[#D7DEE7]">
                    Subprocess / Shell call sink node detected on Line {scanResult.line}. Unescaped string allows command chaining.
                  </p>
                </div>

                {/* Metrics */}
                <div className="p-3 bg-[#030914] border border-[#1E3C5C] space-y-1.5 text-[11px] text-[#8D9AAA]">
                  <div className="flex justify-between">
                    <span>RISK SCORE:</span>
                    <span className="text-[#FF304F] font-bold">{scanResult.risk} / 10.0</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#00CFFF]">{scanResult.entropy} bits/char</span>
                  </div>
                </div>
              ...
              <div className="p-6 text-center space-y-3 font-mono">
                ...
                <div className="text-[10px] text-[#00CFFF]">RISK SCORE: 0.8 / 10.0 (CLEAN)</div>
              </div>
            )}
          </GlassPanel>
        </div>
      ...
      >
        <div className="p-4 bg-[#02050B] border border-[#1E3C5C] font-mono text-xs text-[#8D9AAA] space-y-1">
          <div className="text-[#00CFFF]">&gt; Initializing Python ast.parse(source, mode='exec')...</div>
          ...
          {scanResult.hasVuln ? (
            <div className="text-[#FF304F] font-bold">&gt; [ALERT] SecuritySinkViolation: os.system() forbidden by policy ARG-POL-02</div>
          ) : (
