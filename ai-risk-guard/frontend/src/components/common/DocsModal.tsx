...
import { X, BookOpen, Cpu, ShieldCheck, Box, Terminal, ExternalLink } from 'lucide-react';
...
export const DocsModal: React.FC<DocsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-[#02050B]/90 backdrop-blur-xl animate-fadeIn">
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-[#030914] border border-[#1E3C5C] shadow-[0_0_50px_rgba(8,123,255,0.2)]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1E3C5C] bg-[#06101F]">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="font-mono text-[10px] text-[#8D9AAA]">SPECIFICATION V2.4.0-ENTERPRISE // AUTONOMOUS SECURITY MESH</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tab Navigation */}
        <div className="flex items-center border-b border-[#1E3C5C] bg-[#030914] px-6">
          {[
            ...
            <button
              ...
              className={`flex items-center space-x-2 py-3 px-4 font-mono text-xs tracking-wider border-b-2 transition-all ${
                activeTab === tab.id
                  ? 'border-[#00CFFF] text-[#65E7FF] bg-[#06101F]'
                  : 'border-transparent text-[#8D9AAA] hover:text-white'
              }`}
            >
              {tab.icon}
              <span>{tab.label}</span>
        ...
        {/* Content Area */}
        <div className="p-6 overflow-y-auto space-y-6 text-[#D7DEE7] font-mono text-xs leading-relaxed">
          {activeTab === 'ARCHITECTURE' && (
            <div className="space-y-4">
              <h4 className="font-headline text-base text-white">Autonomous Control Loop Pipeline</h4>
              <p className="text-[#B8C2CE]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 my-4">
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C]">
                  <span className="text-[#00CFFF] font-bold block mb-1">01 DETECTION</span>
                  <span className="text-[#8D9AAA] text-[11px]">Python `ast` AST parser + Shannon entropy regex scanners detect OWASP Top 10 vulnerabilities in milliseconds.</span>
                </div>
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C]">
                  <span className="text-[#087BFF] font-bold block mb-1">02 REMEDIATION</span>
                  <span className="text-[#8D9AAA] text-[11px]">Dual-engine patch pipeline: deterministic `NodeTransformer` rules backed by Google Gemini 1.5 Flash.</span>
                </div>
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C]">
                  <span className="text-[#00E699] font-bold block mb-1">03 VALIDATION</span>
                  <span className="text-[#8D9AAA] text-[11px]">Strict Docker sandbox isolation executes syntax verification, security re-scan, and policy threshold gating.</span>
                </div>
              </div>

              <div className="p-4 bg-[#06101F]/70 border border-[#1E3C5C]">
                <span className="text-[#F1F5F9] font-bold block mb-2">Multi-Agent Orchestration Mesh</span>
                <p className="text-[#8D9AAA] text-[11px]">
                  Coordinated by <span className="text-[#00CFFF]">OrchestrationAgent</span> and <span className="text-[#65E7FF]">ManagerAgent</span>, four specialized worker agents (<span className="text-[#D7DEE7]">ScannerAgent</span>, <span className="text-[#D7DEE7]">PatchAgent</span>, <span className="text-[#D7DEE7]">ValidatorAgent</span>, <span className="text-[#D7DEE7]">RiskAgent</span>) communicate asynchronously over an in-memory event mesh.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'RISK_ENGINE' && (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-[#02050B] border border-[#1E3C5C] text-[#65E7FF]">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-2 bg-[#06101F] border border-[#1E3C5C]/50">
                    <span className="text-[#F1F5F9] font-medium">{f.factor}</span>
                    <div className="flex items-center space-x-3 text-[11px]">
                      <span className="text-[#00CFFF]">WEIGHT: {f.weight}</span>
                      <span className="text-[#8D9AAA]">{f.formula}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'SANDBOX' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-4">
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C] text-center">
                  <span className="text-[#8D9AAA] text-[10px] block">CPU QUOTA</span>
                  <span className="text-[#00CFFF] text-lg font-bold">0.5 CORE</span>
                </div>
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C] text-center">
                  <span className="text-[#8D9AAA] text-[10px] block">MEMORY LIMIT</span>
                  <span className="text-[#00CFFF] text-lg font-bold">128 MB</span>
                </div>
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C] text-center">
                  <span className="text-[#8D9AAA] text-[10px] block">NETWORK ACCESS</span>
                  <span className="text-[#FF304F] text-lg font-bold">DISABLED</span>
                </div>
                <div className="p-3 bg-[#06101F] border border-[#1E3C5C] text-center">
                  <span className="text-[#8D9AAA] text-[10px] block">EXEC TIMEOUT</span>
                  <span className="text-[#00E699] text-lg font-bold">10.0 SEC</span>
                </div>
              </div>

              <div className="p-3 bg-[#030914] border border-[#1E3C5C] text-[11px] text-[#8D9AAA] space-y-1">
                <div><span className="text-[#00CFFF]">SECURITY FLAGS:</span> --pids-limit=32 --read-only --cap-drop=ALL --security-opt=no-new-privileges</div>
                <div><span className="text-[#00CFFF]">ISOLATION:</span> gVisor / runsc kernel syscall filtration sandbox compatible</div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="p-3 bg-[#06101F] border border-[#1E3C5C] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center space-x-2">
                      <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${ep.method === 'POST' ? 'bg-[#087BFF]/20 text-[#00CFFF] border border-[#087BFF]/40' : 'bg-[#00E699]/20 text-[#00E699] border border-[#00E699]/40'}`}>
                        {ep.method}
                      </span>
                      <span className="text-white font-semibold">{ep.path}</span>
                    </div>
                    <span className="text-[#8D9AAA] text-[11px] max-w-md">{ep.desc}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#1E3C5C] bg-[#06101F]">
          <span className="font-mono text-[10px] text-[#8D9AAA]">AUTHENTICATED OPERATOR SPECIFICATION // RESTRICTED ACCESS</span>
